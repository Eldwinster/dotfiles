...
def get_http(url):
    # Required so that urllib3 doesn't give a "403: Forbidden" error
    ...
    http_request = http.request(url, headers=user_agent)

    # Sends the above request and creates a variable with the response
    http_response = urllib3.urlopen(http_request)
...
def get_artist_data(artist):
    ...
    while page <= (total_pictures // 50) + 1:
        ...
        if page == 1:
            ...
            if total_pictures == 0:
                print ('Either the artist has no pictures or (most likely) youve entered an incorrect artist name')
                os._exit(1)
...
def meet_image_conditions(args, json_all_image_data):
    ...
    if not matching_images:
        print ('Your arguments didnt match any of the images from the artist, try changing your requirements')
        os._exit(1)
    else:
...
def download_images(artist_path, matching_images):
    ...
    for image in matching_images:
        ...
        # Get the image via urllib3 then write the data to a file
        else:
            if verbose_mode:
