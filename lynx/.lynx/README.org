...
- on the top/bottom left corners = PREV/NEXT_DOC (i.e., go to the previous document / undo goto previous document);
- on the top/bottom right corners = HISTORY/VLINKS (i.e., call up the history page or visited links page if on history page).
...
**** default cache size and virtual memory size
...
**** Assumed doc charset choice / Display charset choice
...
*** Timeouts
**** infosecs, messagesecs, alertsecs and no pause
...
he styles corresponding to them are present in sample .lss file.
or lynx compiled without lss support, the following settings are the default:
...
**** prettysrc view no anchor numbering
...
**** local execution links always_on/on_but_not_remote
...
**** cookie loose invalid domains, cookie strict invalid domains, cookie query invalid domains
...
**** max cookies domains/global/buffer
MAX_COOKIES_DOMAIN,
...
**** vi keys always on, emacs keys always on
...
**** default keypad mode is number as arrows
...
*** Proxy
**** HTTP/HTTPS/FTP/GOPHER/NEWSPOST/NEWSREPLY/NEWS/NNTP/SNEWSPOST/SNEWSREPLY/SNEWS/WAIS/FINGER/CSO proxy
